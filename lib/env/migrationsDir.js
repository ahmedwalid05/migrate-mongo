...
async function resolveCustomRunner() {
  const { customRunnerPath } = await config.read();
  if (customRunnerPath) {
    if (path.isAbsolute(customRunnerPath)) {
      return customRunnerPath;
    }
    return path.join(process.cwd(), customRunnerPath);
  }
  throw new Error("Couldn't resolve custom runner");
}
...
module.exports = {
  ...
  async loadCustomRunner() {
    const customRunner = await resolveCustomRunner();
    return require(customRunner); // eslint-disable-line
  },
